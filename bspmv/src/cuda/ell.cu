...
template<unsigned int blockSize, unsigned int beh, unsigned int bew>
__global__ void device_cuda_ellpackCM_matrixvector_multithread_simple_mxn(double* as, unsigned int* ja, double* x, double*y, unsigned int max_n_blocks) {
	extern __shared__ double sdata[];
	unsigned int sdata_index = threadIdx.y + threadIdx.x * beh;
	unsigned int bes = beh * bew;
	unsigned int as_off = (blockIdx.x * max_n_blocks + threadIdx.x) * bes + threadIdx.y ;
	unsigned int tid = threadIdx.x;
	sdata[sdata_index] = 0;
	unsigned int ja_off, ja_end;
	unsigned int bid = blockIdx.x;
...
//	printf("0\tblock=%d\tthread=%d\tthreadY=%d\tvalue=%f\n", blockIdx.x, threadIdx.x, threadIdx.y, sdata[sdata_index]);
	for (ja_off = bid * max_n_blocks + tid; ja_off < ja_end; ja_off += blockSize) {
		for (j = 0; j < bew; ++j) {
			sdata[sdata_index] += x[ja[ja_off] + j] * as[as_off + j*beh];
		}
		as_off += bes * blockSize;
	}
...
__host__ double cuda_ellpackCM_matrixvector(unsigned int* h_ja, unsigned int ja_size, double* h_as, unsigned int as_size, unsigned int cols, unsigned int rows, unsigned int beh, unsigned int bew, unsigned int blockRows, unsigned int max_n_blocks, double* h_x, double* h_y, unsigned int blockSize) {
	...
	for (int run = 0; run < TRIES; run++) {
		...
		SWITCH_BLOCKENTRY_SIZE_AND_CUDA_BLOCK_SIZE(blockSize, beh, bew, device_cuda_ellpackCM_matrixvector_multithread_simple_mxn, blockRows, BS, 2 * blockSize * beh * sizeof(double), (d_as, d_ja, d_x, d_y, max_n_blocks))
